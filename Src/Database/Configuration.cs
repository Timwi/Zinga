        ...
        protected override void Seed(Db db)
        {
            db.Puzzles.AddOrUpdate(
                ...
                new Puzzle
                {
                    Title = "Y-Sums Sudoku",
                    Author = "cdwg2000",
                    Links = ClassifyJson.Serialize(new[] { "https://logic-masters.de/Raetselportal/Raetsel/zeigen.php?id=0004A5" }).ToString(),
                    ...
                    ConstraintsJson = ClassifyJson.Serialize(Ut.NewArray(
                    ...
                    ).SelectMany(x => x).ToArray<SvgConstraint>()).ToString()
                }.SetHash(),
                ...
                new Puzzle
                {
                    ...
                    Author = "Timwi",
                    Links = ClassifyJson.Serialize(new[] { "https://logic-masters.de/Raetselportal/Raetsel/zeigen.php?id=0005QA" }).ToString(),
...
The cell with the down-pointing arrow must state how far in the indicated direction the digit 9 is. (For example, a 1 would mean the cell right below it is a 9.)",
                    ConstraintsJson = ClassifyJson.Serialize(Ut.NewArray<SvgConstraint>(
                        new BinairoCage(0, 4),
                        new BinairoCage(5, 4),
                ...
                new Puzzle
                {
                    ...
                    Author = "Brawlbox",
                    Links = ClassifyJson.Serialize(new[] { "https://logic-masters.de/Raetselportal/Raetsel/zeigen.php?id=0003EL" }).ToString(),
                    Rules = @"Normal Sudoku rules apply. Every marked diagonal contains exactly one 1 and one 9. The digits between the 1 and 9 must add up to the number specified. The numbers without arrows are the sums between the 1 and 9 in that row/column.",
                    ConstraintsJson = ClassifyJson.Serialize(Ut.NewArray<SvgConstraint>(
                        new LittleSandwich(DiagonalDirection.SouthEast, 0, 27, display: SvgDiagonalConstraint.DiagonalDisplay.NumberOnly),
